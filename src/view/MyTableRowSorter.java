 ...
 * filtering using a <code>TableModel</code>. The following example shows
 * adding sorting to a <code>JTable</code>:
 * 
 * <pre>
 ...
 * selection model refer to the view and not the underlying model. Therefore, it
 * is necessary to convert between the two. For example, to get the selection in
 ...
 * <code>modelStructureChanged</code> method is invoked) the following are
 * reset to their default values: <code>Comparator</code>s by column, current
 * sort order, and whether each column is sortable. The default sort order is
 ...
 * only contains one type of value, such as an <code>Integer</code>, you
 * should override <code>getColumnClass</code> and return the appropriate
 * <code>Class</code>. This will dramatically increase the performance of
 * this class.
 * 
 * @param <M>
 ...
 * @since 1.6
 */
public class MyTableRowSorter<M extends MyTableModel> extends DefaultRowSorter<M, Integer>
{
	/**
	...
	/**
	 * Sets the <code>TableModel</code> to use as the underlying model for
	 * this <code>TableRowSorter</code>. A value of <code>null</code> can
	 ...
	 *            strings
	 */
	public void setStringConverter(TableStringConverter _stringConverter)
	{
		this.stringConverter = _stringConverter;
	}
	 ...
	 * returned based on the column class (<code>TableModel.getColumnClass</code>)
	 * of the specified column. If the column class is <code>String</code>,
	 * <code>Collator.getInstance</code> is returned. If the column class
	 * implements <code>Comparable</code> a private <code>Comparator</code>
	 * is returned that invokes the <code>compareTo</code> method. Otherwise
	...
	{
		return this.getModel().getRealColumnClass(column);
	}
	 ...
	 * TableModel.
	 */
	class TableRowSorterModelWrapper extends ModelWrapper<M, Integer>
	{
		public M getModel()
		{
			return tableModel;
		...
		{
			return tableModel.getRealValueAt(row, column);
		}
